...
<!-- census:begin -->
- **15 of 165** admitted records are published — this catalog is audited-only — **13** on the full twelve-field ladder and **2** as `audited-lite` (six fields established from the page, `build_is_real`/`stack_consistency` unexamined — see ADR-P15). **150** sit in `data/pool.json`: 7 captured, scored and held for cause · 143 never captured at all.
- **22** project pages have been deep-captured, 20 of them with a recorded figures table (claims written down with a denominator — whether they then proved checkable is each record's `numbers_add_up`), and 1 linking a repository we could verify against `api.github.com`. Hazard notes sit on 5 published and 2 held records.
- 125 pipeline tests, every surface regenerated by `make build`, and `make verify` fails the build if this block and `catalog-stats.json` ever disagree — including this block.
<!-- census:end -->
