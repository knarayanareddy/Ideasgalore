...
- 115 pipeline tests, every surface regenerated by `make build`, and `make verify` fails the build if this block and `catalog-stats.json` ever disagree — including this block.
<!-- census:end -->
...
the ladder for both tiers so an agent can tell what a lite row would take to become full. Until P15 lands,
the twelve-field rule stands unmodified — the rails above raise *how many* get audited, not how thin an
audit may be.
