...
<!-- pool-split:begin -->
- `data/pool.json` holds all 153 held-out records and `provenance` splits them: 145 `unaudited` rows nobody has captured, 8 `audited-hold` rows that were captured, scored and held back for cause. A held row lists its blockers in `why_not_promoted[]` (2 to 6 entries) and what would clear them in `would_settle_it[]` (1 to 5 entries).
<!-- pool-split:end -->
