...
- **A hazard follows the claim, not the shelf.** `hazard_for` fires on a regulated-outcome claim
  *or* on advice inside a hazard sector, and batch 5 added the first half's missing register: naming a
  ...
  uses the word "compliance". This is not cosmetic: correcting Carbender's sector from Climate to
...
```jsonc
// data/audits.json — one entry per published record (11 rows), keyed by id;
  ...
  "candidates": [
   {
    "id": "tower-dq18x2",
    "name": "Tower",
    "domain": "Agentic Autonomy & Orchestration",
    "coolness": 0.396,
    "expected_gain": 0.5212,
    "why": "mechanism-rich hook; depth likely to convert",
    "would_settle_it": [
...
<!-- pool-split:begin -->
- `data/pool.json` holds all 154 held-out records and `provenance` splits them: 147 `unaudited` rows nobody has captured, 7 `audited-hold` rows that were captured, scored and held back for cause. A held row lists its blockers in `why_not_promoted[]` (2 to 7 entries) and what would clear them in `would_settle_it[]` (1 to 5 entries).
<!-- pool-split:end -->
...
<!-- coverage:begin -->
- `pool_size: 147` is the honest denominator and the `note` says what it excludes. Coverage here is 11 of 165 admitted records audited.
<!-- coverage:end -->
...
- It cannot see a repository that a page does not link. Of the 18 records deep-captured so far, exactly one — SATU — printed a repository URL, and it is the only record in this catalog whose `build_is_real` could be `confirmed` and whose verdict reached `strong` (§4).
  The other 17 sit at `unverifiable` on that check no matter how careful their prose; the bulk listing rows carry no links at all, because the gallery scrape never included them, so a row learns its `repo_url`, `demo_url` and `video_url` only when a capture is folded in at ingest. The audit's reach is therefore a function of what teams chose to link, not of how well they wrote.
<!-- repo-blindness:end -->
...
<!-- corpus-bias:begin -->
- It inherits the corpus's biases. 85 of the 165 admitted rows come from one gallery (`Build with Gemini XPRIZE`, 1,401 projects across 59 gallery pages, of which 4 were crawled); 18 records have been captured for audit, and 165 admitted rows are what the whole corpus holds. `catalog-stats.json → coverage` publishes the same figures so a consumer cannot quietly forget them.
<!-- corpus-bias:end -->
