...
```jsonc
// data/audits.json — one entry per published record (14 rows), keyed by id;
...
// data/promotion-queue.json — pool_size counts only records with no sheet at all
{
  ...
  "candidates": [
   {
    "id": "zeroday-ai-autonomous-vulnerability-patching-system",
    "name": "ZeroDay AI",
    "domain": "Developer Tooling & Code Intelligence",
    "coolness": 0.359,
    "expected_gain": 0.514,
    "why": "sector Developer Tooling & Code Intelligence has 6 rows and no audit",
    "would_settle_it": [
...
<!-- coverage:begin -->
- `pool_size: 144` is the honest denominator and the `note` says what it excludes. Coverage here is 14 of 165 admitted records audited.
<!-- coverage:end -->
...
- It cannot see a repository that a page does not link. Of the 21 records deep-captured so far, exactly one — SATU — printed a repository URL, and it is the only record in this catalog whose `build_is_real` could be `confirmed` and whose verdict reached `strong` (§4).
  The other 20 sit at `unverifiable` on that check no matter how careful their prose; the bulk listing rows carry no links at all, because the gallery scrape never included them, so a row learns its `repo_url`, `demo_url` and `video_url` only when a capture is folded in at ingest. The audit's reach is therefore a function of what teams chose to link, not of how well they wrote.
<!-- repo-blindness:end -->
...
<!-- corpus-bias:begin -->
- It inherits the corpus's biases. 85 of the 165 admitted rows come from one gallery (`Build with Gemini XPRIZE`, 1,401 projects across 59 gallery pages, of which 4 were crawled); 21 records have been captured for audit, and 165 admitted rows are what the whole corpus holds. `catalog-stats.json → coverage` publishes the same figures so a consumer cannot quietly forget them.
<!-- corpus-bias:end -->
